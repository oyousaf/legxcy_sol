...
export default function About() {
  ...
  return (
    ...
    >
      {/* Gradient overlay */}
      <div className="absolute inset-0 bg-linear-to-b from-(--dark-mint)/95 via-(--dark-mint)/85 to-(--mossy-bg)/95 z-0" />

      {/* Floating parallax icons */}
      <motion.div
        style={{ y: y1 }}
        className="absolute top-16 left-6 text-(--accent-green) opacity-25 text-6xl pointer-events-none"
      >
        <FaBolt />
      </motion.div>
      <motion.div
        style={{ y: y2 }}
        className="absolute bottom-20 right-10 text-(--accent-green) opacity-20 text-7xl pointer-events-none"
      >
        <FaLayerGroup />
      </motion.div>
      ...
      <div className="max-w-6xl mx-auto text-center relative z-10">
        ...
        <motion.p
          ...
          viewport={{ once: true }}
          className="text-lg sm:text-xl max-w-3xl mx-auto mb-16 text-(--foreground) leading-relaxed"
        >
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
          {highlightItems.map((item, index) => (
            <motion.div
              ...
              className="relative bg-white/5 backdrop-blur-lg border border-white/10 hover:border-(--accent-green) p-6 rounded-2xl shadow-lg transition-all duration-300 group"
            >
